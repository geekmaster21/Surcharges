...
export const Dotize = {
    ...
    getFieldName: function (field: string, prefix: string, isRoot?: boolean, isArrayItem?: boolean, isArray?: boolean) {
        if (isArray)
    ...
    convert: function (obj: any, prefix?: string) {
        ...
        return function recurseConvert(o: any, p: string | undefined, isRoot?: boolean) {
                ...
                if (currentProp && typeof currentProp === "object") {
                    if (Array.isArray(currentProp)) {
                        if (Dotize.isEmptyArray(currentProp)) {
                            newObj[Dotize.getFieldName(f, _prefix, isRoot, false, true)] = currentProp;
                        } else {
                            newObj = recurseConvert(currentProp, Dotize.getFieldName(f, _prefix, isRoot, false, true), isArrayItem); // array
                        }
                    } else {
                        if (isArrayItem && Dotize.isEmptyObj(currentProp) === false) {
                            newObj = recurseConvert(currentProp, Dotize.getFieldName(f, _prefix, isRoot, true)); // array item object
                        } else if (Dotize.isEmptyObj(currentProp) === false) {
                            newObj = recurseConvert(currentProp, Dotize.getFieldName(f, _prefix, isRoot)); // object
                        } else if (Dotize.isEmptyObj(currentProp)) {
                            newObj[Dotize.getFieldName(f, _prefix, isRoot, isArrayItem)] = currentProp;
                        }
                    }
                } else {
