...
export const SetCurrentLocale = (locale: string) => {
  ...
  cookie.set(CooKEY, locale, { expires: 1000, sameSite: 'Lax' });
};

export const GetCurrentLocale = (firstOrDefault = true): string => {
  if (!firstOrDefault) {
    return cookie.get(CooKEY) as string;
  }
  const locale = cookie.get(CooKEY) || config.locale.current;
